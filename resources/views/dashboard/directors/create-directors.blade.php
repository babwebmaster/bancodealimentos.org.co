<x-app-layout>
    ...
    {{ Breadcrumbs::render('directors.create', $director) }}
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form action="{{  route('directors.store', $director->id)  }}" class="px-4 py-4 w-full md:w-3/6 md:mx-auto bg-white-4/5 rounded-md" method="POST" enctype="multipart/form-data">
                @csrf
                <h3 class="text-center text-2xl font-bold mb-5">{{  __('messages.Create director')  }} #{{  $director->id  }}</h3>
                <div class="mb-5">
                    <label for="valor" class="mb-3 block text-base font-medium text-black">
                        {{  __('messages.Director title')  }}
                    </label>
                    <input type="text" name="title" id="title" placeholder="{{  __('messages.Director title')  }}" value="{{  old("title", $director->title)  }}"
                ...
                <div class="mb-5">
                    ...
                    <input type="text" name="name" id="name" placeholder="{{  __('messages.Director name')  }}" value="{{  old("name", $director->name)  }}"
                        class="w-full rounded-md border border-[#e0e0e0] bg-transparent py-3 px-6 text-base font-medium text-gry-900 outline-none focus:border-[#6A64F1] focus:shadow-md" />
                    @if ($errors->has('name'))
                ...
                <div class="my-5">
                    ...
                    <img src="{{  $director->image  }}" class="h-20 aspect-square hidden" id="preview" alt="{{  $director->name  }}">
                    <br>
                ...
                <div class="mb-5">
                    ...
                    <textarea name="description" id="description" placeholder="{{  __('messages.Director description')  }}" value=""
                        class="w-full rounded-md border border-[#000] bg-transparent text-bab py-3 px-6 text-base text-left font-medium text-bab outline-none focus:border-[#6A64F1] focus:shadow-md">{{  old("name", $director->description)  }}</textarea>
