<x-app-layout>
    ...
    {{ Breadcrumbs::render('cifras.create', $cifra) }}
    <div class="mb-5">
        <div class="max-w-full mx-auto py-1 sm:px-6 lg:px-8">
            <form action="{{  route('cifras.store', $cifra->id)  }}" class="px-4 py-4 w-full md:w-3/6 md:mx-auto bg-white-4/5 rounded-md" method="POST" enctype="multipart/form-data">
                @csrf
                <h3 class="text-center text-2xl font-bold">{{  __('messages.Create figure')  }}</h3>
                <div class="mb-5">
                    <label for="name" class="mb-3 block text-base font-medium text-black">
                        {{  __('messages.Figure name')  }}
                    </label>
                    <input type="text" name="name" id="name" placeholder="{{  __('messages.Figure name')  }}" value="{{  old("name", $cifra->name)  }}"
                ...
                <div class="mb-5">
                    ...
                    <input type="number" name="value" id="value" placeholder="{{  __('messages.Figure value')  }}" value="{{  old("value", $cifra->value)  }}"
                        class="w-full rounded-md border border-[#e0e0e0] bg-transparent py-3 px-6 text-base font-medium text-gry-900 outline-none focus:border-[#6A64F1] focus:shadow-md" />
                    @if ($errors->has('value'))
                ...
                <div class="my-5">
                    ...
                    <img src="{{  $cifra->icon  }}" class="h-20 aspect-square hidden" id="preview" alt="{{  $cifra->name  }}">
                    <br>
