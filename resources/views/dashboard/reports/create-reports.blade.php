    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form action="{{  route('reports.store', $report->id)  }}" class="px-4 py-4 w-full md:w-3/6 md:mx-auto bg-white-4/5 rounded-md" method="POST" enctype="multipart/form-data">
                @csrf
                <h3 class="text-center text-2xl font-bold mb-5">{{  __('messages.Create management report')  }}</h3>
                <div class="mb-5">
                    <label for="name" class="mb-3 block text-base font-medium text-black">
                        {{  __('messages.Report name')  }}
                    </label>
                    <input type="text" name="name" id="name" placeholder="{{  __('messages.Report name')  }}" value="{{  old("linkSpanish", $report->name)  }}"
                ...
                <div class="my-5">
                    ...
                    <img src="{{  $report->image  }}" class="h-20 aspect-square hidden" id="preview" alt="{{  $report->name  }}">
                    <br>
                ...
                <div class="mb-5">
                    ...
                    <input type="text" name="linkSpanish" id="linkSpanish" placeholder="{{  __('messages.Report link spanish')  }}" value="{{  old("linkSpanish", $report->linkSpanish)  }}"
                        class="w-full rounded-md border border-[#e0e0e0] bg-transparent py-3 px-6 text-base font-medium text-gry-900 outline-none focus:border-[#6A64F1] focus:shadow-md" />
                ...
                <div class="mb-5">
                    ...
                    <input type="text" name="linkEnglish" id="linkEnglish" placeholder="{{  __('messages.Report link english')  }}" value="{{  old("linkEnglish", $report->linkEnglish)  }}"
                        class="w-full rounded-md border border-[#e0e0e0] bg-transparent py-3 px-6 text-base font-medium text-gry-900 outline-none focus:border-[#6A64F1] focus:shadow-md" />
                ...
                <div class="mb-5">
                    ...
                    <input type="text" name="linkFrench" id="linkFrench" placeholder="{{  __('messages.Report link french')  }}" value="{{  old("linkFrench", $report->linkFrench)  }}"
                        class="w-full rounded-md border border-[#e0e0e0] bg-transparent py-3 px-6 text-base font-medium text-gry-900 outline-none focus:border-[#6A64F1] focus:shadow-md" />
